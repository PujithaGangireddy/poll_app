...
func (vq *VoteQuery) QueryOptions() *PollOptionQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, vote.OptionsTable, vote.OptionsColumn),
		)
		fromU = sqlgraph.SetNeighbors(vq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (vq *VoteQuery) Clone() *VoteQuery {
	...
	return &VoteQuery{
		...
		withOptions: vq.withOptions.Clone(),
...
func (vq *VoteQuery) WithOptions(opts ...func(*PollOptionQuery)) *VoteQuery {
	query := (&PollOptionClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withOptions = query
	return vq
}
...
func (vq *VoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vote, error) {
	...
	if query := vq.withOptions; query != nil {
		if err := vq.loadOptions(ctx, query, nodes,
			func(n *Vote) { n.Edges.Options = []*PollOption{} },
			func(n *Vote, e *PollOption) { n.Edges.Options = append(n.Edges.Options, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VoteQuery) loadOptions(ctx context.Context, query *PollOptionQuery, nodes []*Vote, init func(*Vote), assign func(*Vote, *PollOption)) error {
	...
	query.Where(predicate.PollOption(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(vote.OptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.vote_options
		if fk == nil {
			return fmt.Errorf(`foreign-key "vote_options" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "vote_options" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
