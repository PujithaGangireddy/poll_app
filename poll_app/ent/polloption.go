...
func (*PollOption) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case polloption.ForeignKeys[0]: // poll_options
			values[i] = new(sql.NullInt64)
		case polloption.ForeignKeys[1]: // vote_options
			values[i] = new(sql.NullInt64)
		default:
...
func (po *PollOption) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case polloption.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field vote_options", value)
			} else if value.Valid {
				po.vote_options = new(int)
				*po.vote_options = int(value.Int64)
			}
		default:
