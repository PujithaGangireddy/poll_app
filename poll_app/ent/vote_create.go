...
func (vc *VoteCreate) AddOptionIDs(ids ...int) *VoteCreate {
	vc.mutation.AddOptionIDs(ids...)
	return vc
}

// AddOptions adds the "options" edges to the PollOption entity.
func (vc *VoteCreate) AddOptions(p ...*PollOption) *VoteCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return vc.AddOptionIDs(ids...)
}
...
func (vc *VoteCreate) createSpec() (*Vote, *sqlgraph.CreateSpec) {
	...
	if nodes := vc.mutation.OptionsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
